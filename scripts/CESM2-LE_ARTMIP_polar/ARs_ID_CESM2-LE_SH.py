...
def ARs_ID(AR_config, begin_time, end_time, timestep_hrs, ens_member):
    """
    ...
    IVT_at_pctiles_ds_all_lats = rename_coords(xr.open_dataset(IVT_at_pctiles_fpath))
    IVT_at_pctiles_ds = IVT_at_pctiles_ds_all_lats.where(
        IVT_at_pctiles_ds_all_lats['lat'] <= AR_config['max_lat'], drop=True)
    ...
    for i,t in enumerate(times):
        ...
        label_array_prelim, IVT_wrap, IVT_at_thresh_wrap, u_wrap, v_wrap, lons_wrap = \
            build_wrap_arrays(AR_config, ix_df_t, IVT_at_pctiles_ds_doy, ll_mean_wind)
...
def build_input_file_indexing_df(AR_config, times, ens_member,
                                 ll_mean_wind=False):
    """
...
def _sift_fpaths(AR_config, data_dir, begin_dt, end_dt, ens_member):
    """
    ...
    for fpath in fpaths_all:
        ...
        start_yr = dt.datetime.strptime(fname.split('_')[-2], 
                                        AR_config['IVT_fname_date_format']).year
        end_yr = dt.datetime.strptime(fname.split('_')[-1], 
                                      AR_config['IVT_fname_date_format']+'.nc').year
        file_ens_member = \
            fname.split(f'{AR_config['data_source']}-')[1].split('_')[0]
        if ((start_yr in analysis_yrs) or (end_yr in analysis_yrs)) and \
            (file_ens_member == ens_member):
            fpaths.append(fpath)
...
def build_wrap_arrays(AR_config, ix_df_t, IVT_at_pctiles_ds_doy, ll_mean_wind):    
    """
    ...
    IVT_ds_all = rename_IVT_components(rename_coords(xr.open_dataset(ix_df_t['IVT_fpath'])))
    IVT_ds_all_lats = IVT_ds_all.sel(time=ix_df_t.t)
    IVT_ds = IVT_ds_all_lats.where(
        IVT_ds_all_lats['lat'] <= AR_config['max_lat'], drop=True)
    ...
    IVT_PR_thresh_str = f'{AR_config['IVT_PR_thresh']:.1f}'
    IVT_at_thresh = IVT_at_pctiles_ds_doy[f'IVT_pctile_{IVT_PR_thresh_str}']
...
def write_AR_labels_file(AR_config, begin_t, end_t, timestep_hrs,
    ...
    ARs_ds.attrs['ens_member'] = ens_member
    ...
    if (minlat == 10) and (maxlat == 90):
        fname = f'ARs_{data_source}_{ens_member}_NH_{timestep_hrs}hr_{t_begin_str}_{t_end_str}.nc'
    elif (minlat == -90) and (maxlat == -10):
        fname = f'ARs_{data_source}_{ens_member}_SH_{timestep_hrs}hr_{t_begin_str}_{t_end_str}.nc'  
    else:
        fname = f'ARs_{data_source}_{ens_member}_lat_{minlat}_{maxlat}_{timestep_hrs}hr_{t_begin_str}_{t_end_str}.nc'  
...
def parse_args():
    ...
    return args.begin_time, args.end_time, args.timestep_hrs,\
        args.AR_ID_config_path, args.ens_member
...
def main():
    ...
    begin_time, end_time, timestep_hrs, AR_ID_config_path, ens_member = \
        parse_args()
    ...
    AR_labels, times, lats, lons, climo_start_year, climo_end_year, climo_timestep_hrs = \
        ARs_ID(AR_config, begin_time, end_time, timestep_hrs, ens_member)
