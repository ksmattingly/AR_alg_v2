...
def calc_IVT_at_pctiles(AR_config, start_year, end_year, timestep_hrs,
                        start_doy, end_doy, ens_member):
    """
    Calculate IVT values at a range of percentiles for each julian day of the year.
    """
    
    IVT_dir = AR_config['IVT_dir']
    IVT_paths = sorted(glob.glob(IVT_dir+f'*LE-{ens_member}*.nc'))
    ...
    end_dt = dt.datetime(int(end_year), 12, 31, 18)
    ...
    IVT_ds_all_lats = IVT_ds_full.IVT.sel(time=slice(
        begin_dt.strftime('%Y-%m-%d'), end_dt.strftime('%Y-%m-%d')))
...
def write_IVT_at_pctiles_output_file(AR_config, start_year, end_year, timestep_hrs,
    ...
    IVT_at_pctiles_ds.attrs['ens_member'] = ens_member
    ...
    if ((minlat == -90) and (maxlat == 90) and ((maxlon+lonres) - minlon == 360)) and\
        ...
        fname = 'IVT_at_pctiles_'+AR_config['data_source']+\
            f'_{ens_member}_global_{timestep_hrs}hr_{start_year}_{end_year}_climo.nc'    
    elif ((minlat == 10) and (maxlat == 90) and ((maxlon+lonres) - minlon == 360)) and\
        (start_doy == '1' and end_doy == '365'):
        fname = 'IVT_at_pctiles_'+AR_config['data_source']+\
            f'_{ens_member}_NH_{timestep_hrs}hr_{start_year}_{end_year}_climo.nc'  
    elif ((minlat == -90) and (maxlat == -10) and ((maxlon+lonres) - minlon == 360)) and\
        (start_doy == '1' and end_doy == '365'):
        fname = 'IVT_at_pctiles_'+AR_config['data_source']+\
            f'_{ens_member}_SH_{timestep_hrs}hr_{start_year}_{end_year}_climo.nc'  
    else:
        fname = 'IVT_at_pctiles_'+AR_config['data_source']+\
            f'_{ens_member}_lat_{minlat}_{maxlat}_lon_{minlon}_{maxlon}_{timestep_hrs}hr_doys_{start_doy}_{end_doy}_{start_year}_{end_year}_climo.nc' 
...
def parse_args():
    ...
    return args.start_doy, args.end_doy, args.ens_member,\
        args.AR_ID_config_path


def main():
    start_doy, end_doy, ens_member, AR_ID_config_path = parse_args()
    ...
    write_IVT_at_pctiles_output_file(
        ...
        lats, lons, IVT_pctiles_out_data, ens_member
        )
